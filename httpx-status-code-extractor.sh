...
then
    ...
    echo "Usage: fuzz-Extractor.sh <file(subdomain-file.txt)>"
    exit 1
fi

file_name=$(echo $1 | sed "s/.txt//g")

echo "Start Extracting result of httpx with status code [20X, 30X, 401,403, 500] with txt extension"
sleep 2

# Extract 20X responses
cat $1 | grep -E "(20[0-6]{1})" | cut -d " " -f1 > $file_name.20x.txt

# Extract 30X responses
cat $1 | grep -E "(30[0-7]{1})" | cut -d " " -f1 > $file_name.30x.txt

# Extract 4XX responses
cat $1 | grep -E "(4[0-1]{1}[0-9]{1})" | cut -d " " -f1 > $file_name.4xx.txt

# Extract 50X responses
cat $1 | grep -E "(50[0-5]{1})" | cut -d " " -f1 > $file_name.50x.txt
